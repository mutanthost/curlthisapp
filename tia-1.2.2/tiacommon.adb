...
function IsLoadException( current, newfile : str255 ) return boolean is
  Key : Str255;
  Location : Str255List.AListIndex := 0; -- 0 just to suppress warning
begin
  Key := current & "~" & newfile;
...
begin
  ...
  if length( fragment ) = 0 then
     ...
     -- unfortunately, gnatmake 4 or greater returns an error for the version
  ...
  if length( fragment ) = 0 then
     fragment := to255("unknown");
  end if;
  draftsig := draftsig & fragment;
